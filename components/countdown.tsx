...
type Parts = { days: number; hours: number; minutes: number; seconds: number; isToday: boolean }
...
export default function Countdown() {
  ...
  useEffect(() => {
    const tick = () => setParts(computeParts(new Date()))
    tick()
  ...
  return (
    <div className="rounded-2xl border border-emerald-400/20 bg-black/40 p-4 backdrop-blur-md sm:p-5">
      ...
      <p className="mb-3 text-sm text-emerald-100/50">22 Ekim</p>

      {parts.isToday ? (
        <p className="py-2 text-lg font-bold text-emerald-300">İyi ki doğdun! 🎉</p>
      ) : (
        <div className="flex items-start gap-3 sm:gap-4">
          <Unit value={parts.days} label="Gün" />
          <span className="pt-1 text-2xl text-emerald-400/40 sm:text-3xl">:</span>
          <Unit value={parts.hours} label="Saat" />
          <span className="pt-1 text-2xl text-emerald-400/40 sm:text-3xl">:</span>
          <Unit value={parts.minutes} label="Dk" />
          <span className="pt-1 text-2xl text-emerald-400/40 sm:text-3xl">:</span>
          <Unit value={parts.seconds} label="Sn" />
        </div>
      )}
    </div>
  )
